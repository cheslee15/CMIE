...
def direct_reasoning(args, caption, image_path):
    for repeat in range(args.max_repeat):
        try:
            response = json.loads(extract_json_string(direct_prompt(caption, image_path)))
            print(response)
            return response
        except:
            continue
...
def evidence_enhanced_reasoning(args, caption, image_path, img_evidence_list):
    for repeat in range(args.max_repeat):
        try:
            response = json.loads(extract_json_string(evidence_enhanced_prompt(caption, image_path, img_evidence_list)))
            print(response)
            return response
        except:
            continue
...
def CMIE_reasoning(caption, image_path, visual_evidence, entity):
    for repeat in range(args.max_repeat):
        try:
            coexistence = json.loads(extract_json_string(coexistence_prompt(caption, image_path)))
            ...
            response = json.loads(extract_json_string(Final_Pred(caption, coexistence, image_title_score, entity, image_path)))
            print(response)
            return response
        except:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--methods', type=str, default='4o-CMIE', help='base model', choices=['4o-direct','4o-evidence-enhanced', '4o-CMIE'])
